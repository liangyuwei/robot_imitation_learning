...
%% Prep
file_name = '../motion-retargeting/mocap_ik_results_YuMi_g2o_similarity.h5'; %'../motion-retargeting/dmp_optimize_results-.h5';
...
%% Load data from liweijie's result
%{
...
%% Arrange moved and optimized DMP starts and goals into history data
% initialization
...
% get trajectories
y_r_wrist_history = zero(3, num_resampled_points, num_iters);
y_l_wrist_history = zero(3, num_resampled_points, num_iters);
y_r_elbow_history = zero(3, num_resampled_points, num_iters);
y_l_elbow_history = zero(3, num_resampled_points, num_iters);
for i = 1 : num_iters
    ...
    y_l_elbow_history(:, :, i) = y_l_wrist + y_lew;    
end
...
for i = 1 : num_iters
    ...
    y_r_wrist = y_r_wrist(:, :, i);
    y_l_wrist = y_l_wrist(:, :, i);
    y_r_elbow = y_r_elbow(:, :, i);
    y_l_elbow = y_l_elbow(:, :, i);
    % plot it
...
for i = 1 : num_iters
    ...
    % re
    tmp_re_max = [tmp_re_max, max(y_r_erbow_history(:, :, i), [], 2)];
    tmp_re_min = [tmp_re_min, min(y_r_erbow_history(:, :, i), [], 2)];
end
