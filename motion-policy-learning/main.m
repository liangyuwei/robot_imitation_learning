...
l_x_mid = [0.5, 0.32, 0.35;...
           0.5, 0.2263, 0.5763; ...
           0.2848, 0.2152, 0.4889; ...
           0.5476, 0.3006, 0.2511; ...
           0.2593, 0.1749, 0.2322; ...
           0.6294, 0.2540, 0.4953; ...
           0.4595, 0.2557, 0.5381];
l_x_final = [0.5, 0.12, 0.35;...
             0.5, 0.0849, 0.4349; ...
             0.4193, 0.0807, 0.3871; ...
             0.5179, 0.1127, 0.3129; ...
             0.4097, 0.0656, 0.3058; ...
             0.5485, 0.0953, 0.4045; ...
             0.4848, 0.0959, 0.4205];
...
l_eul_final = l_eul_mid; %[]; % - xyz, for RViz
...
r_x_mid = [0.5, -0.29, 0.35;...
           0.5, -0.2051, 0.1449; ...
           0.6950, -0.1950, 0.2604; ...
           0.4569, -0.2724, 0.4396; ...
           0.7181, -0.1585, 0.4568; ...
           0.3827, -0.2302, 0.2183; ...
           0.5367, -0.2317, 0.1795];
r_x_final = [0.5, -0.19, 0.35;...
             0.5, -0.1344, 0.2156;...
             0.6278, -0.1278, 0.2913; ...
             0.4717, -0.1785, 0.4087; ...
             0.6429, -0.1038, 0.4199; ...
             0.4231, -0.1508, 0.2637; ...
             0.5240, -0.1518, 0.2383];
...
r_eul_final = r_eul_mid; %[]; % - xyz, for RViz
n_samples = size(l_x_start, 1);
wrist_traj_dataset_tmp = cell(n_samples, 2);
for n = 1 : n_samples
    disp(['Generating fake trajectory ', num2str(n), '/', num2str(n_samples), '...']);
    wrist_traj_dataset_tmp{n, 1} = generate_fake_eef_traj([l_x_start(n, :), l_eul_start(n, :)], ...
                                                      [l_x_mid(n, :), l_eul_mid(n, :)], ...
                                                      [l_x_final(n, :), l_eul_final(n, :)], ...
                                                      1000);
    wrist_traj_dataset_tmp{n, 2} = generate_fake_eef_traj([r_x_start(n, :), r_eul_start(n, :)], ...
                                                      [r_x_mid(n, :), r_eul_mid(n, :)], ...
                                                      [r_x_final(n, :), r_eul_final(n, :)], ...
                                                      1000);
end
wrist_traj_dataset_aligned = [wrist_traj_dataset_aligned; wrist_traj_dataset_tmp];
tmp = t_series_aligned;
num = size(wrist_traj_dataset_aligned, 1);
t_series_aligned = cell(num, 1);
for j = 1 : num
    t_series_aligned{j, 1} = tmp{randi(8), 1};
end
...
suptitle('Display of orientation change.');
%}
disp('Done.');
