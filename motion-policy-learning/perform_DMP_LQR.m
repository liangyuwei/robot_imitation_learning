function r = perform_DMP_LQR(traj_dataset, nbStates, nbVar, nbVarPos, kP, kV, alpha, dt, nbData, nbSamples, new_goal, new_start)
...
model.nbVar = 7;%nbVar; %Number of variables [s,F1,F2] (decay term and perturbing force)
...
model.alpha = alpha; %Decay factor
...
nbData = nbData; %Number of datapoints in a trajectory
...
for n=1:nbSamples
	...
	s(n).Data = spline(1:size(traj_dataset{n}',2), traj_dataset{n}', linspace(1,size(traj_dataset{n}',2),nbData)); %Resampling
...
% 	ddx = K * ([r(1).currTar(:,t); zeros(model.nbVarPos,1)] - [x; dx]);
	ddx = K * ([new_goal; zeros(model.nbVarPos,1)] - [x; dx]);
	dx = dx + ddx * model.dt;
	x = x + dx * model.dt;
	r(1).Data(:,t) = x;
end	
