...
%            obj.ur5_kin = UR5Kinematics();
            ...
            obj.precomp_matrices{1} = makehgtform('xrotate', 0); 
            % done!
            %makehgtform('xrotate', -pi/2) *  makehgtform('zrotate', pi); 
            obj.precomp_matrices{2} = makehgtform('yrotate', -pi/2) * makehgtform('translate', [0 0 -obj.offset_2]);
            % done!
            %makehgtform('xrotate', pi/2) * makehgtform('yrotate', -pi/2) * makehgtform('translate', [0 obj.offset_1 -obj.offset_2]);
            obj.precomp_matrices{3} = makehgtform('yrotate', pi/2); %* makehgtform('translate', [0.4 obj.offset_7 -obj.offset_3]);
            % done!
            %makehgtform('xrotate', pi/2) * makehgtform('yrotate', -pi/2) * makehgtform('translate', [0 obj.offset_7 -obj.offset_3]);
            obj.precomp_matrices{4} = makehgtform('yrotate', pi);
            % done!
            % makehgtform('translate', [0 -obj.offset_5 0]);
            obj.precomp_matrices{5} = makehgtform('yrotate', pi);
            % done!
            %makehgtform('xrotate', pi/2) * makehgtform('translate', [0 0 -obj.offset_5]);
...
%           % from q to transformatrices, using my code 
            global uLINK th            
            % assign joint angles
            th = q'; 
            for nq = 1 : 6
...
%                 end
            ...
            ForwardKinematics(2);
            T1 = [[uLINK(2).R, uLINK(2).p]; 0, 0, 0, 1]; 
            T2 = [[uLINK(3).R, uLINK(3).p]; 0, 0, 0, 1]; 
            T3 = [[uLINK(4).R, uLINK(4).p]; 0, 0, 0, 1]; 
            T4 = [[uLINK(5).R, uLINK(5).p]; 0, 0, 0, 1]; 
            T5 = [[uLINK(6).R, uLINK(6).p]; 0, 0, 0, 1]; 
            T6 = [[uLINK(7).R, uLINK(7).p]; 0, 0, 0, 1]; 
