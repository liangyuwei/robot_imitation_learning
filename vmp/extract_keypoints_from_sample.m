function kp_id_list = extract_keypoints_from_sample(l_wrist_pos, l_wrist_quat, l_elbow_pos, r_wrist_pos, r_wrist_quat, r_elbow_pos, l_glove_angle, r_glove_angle)
%% extract_keypoints_from_sample.m
% should be of the size(DOF, length)
...
for i = 1:length
    % left
    eul = quat2eul(l_wrist_quat(:, i)');
    l_wrist_euler(:, i) = eul';
    % right
    eul = quat2eul(r_wrist_quat(:, i)');
    r_wrist_euler(:, i) = eul';    
end
...
eps11 = 3; % sufficient time passed since last keypoint
...
eps14 = 3; % sufficient time passed
...
%% Output constraints
kp_id_list = kp_indices;
% unique([l_wrist_pos_kp_id_list, r_wrist_pos_kp_id_list, ...
